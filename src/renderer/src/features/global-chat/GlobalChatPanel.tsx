...
import { MessageSquare, FolderOpen, Plus, GripVertical } from 'lucide-react'
...
import { useGlobalChatStore } from './state/chatStore'
...
import { useProjectChat } from '@/features/workspace/hooks/useProjectChat'
...
/**
 * GlobalChatPanel - 全局对话面板（挤压式）
 *
 * 从右侧挤压主内容区域的对话面板
 * 支持选择项目、管理会话、发送消息、拖拽调整宽度
 */
  ...
  if (!selectedProject) {
    return (
      ...
      >
        {/* 拖拽手柄 */}
        <div
          className="absolute left-0 top-0 z-10 flex h-full w-1 cursor-col-resize items-center justify-center hover:bg-primary/20 active:bg-primary/30"
        ...
        <div className="flex items-center justify-between border-b px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">{t('globalChat.title', '对话')}</span>
          </div>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onMouseDown={handleMouseDown}
      >
        <GripVertical className="h-4 w-4 text-muted-foreground/30 opacity-0 hover:opacity-100" />
      </div>
...
function GlobalChatContent({
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {/* Messages */}
      <div className="flex flex-1 flex-col min-h-0 overflow-hidden px-3 pt-3">
