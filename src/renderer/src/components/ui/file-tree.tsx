        ...
        <div className={cn('size-full', className)} {...props}>
          <ScrollArea ref={ref} className="relative h-full px-2" dir={dir as Direction}>
            <AccordionPrimitive.Root
              ...
              onValueChange={(value) => setExpandedItems((prev) => (prev === value ? prev : value))}
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <div
        ...
        ref={ref}
        className={cn(
          'bg-border/60 absolute left-1.5 h-full w-px rounded-md py-3 duration-300 ease-in-out rtl:right-1.5',
          className
        )}
        {...props}
      />
...
const Folder = ({
  ...
  const { direction, expandedItems, indicator, setExpandedItems, openIcon, closeIcon } = useTree()
...
const CollapseButton = ({
  ...
  return (
    <Button
      variant={'ghost'}
      className={cn('absolute right-2 bottom-1 h-8 w-fit p-1', props.className)}
