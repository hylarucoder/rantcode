...
- **ActivityBar**：支持多视图切换的导航系统（sessions/assistant/docs/git/settings）

### 下一步重点

1. **Task 看板实现**：基于现有 Session 架构构建 Task 管理功能
2. **Session-Task 关联**：建立会话与任务的关联关系
3. **结构化存储**：完善 Job 实体的持久化存储
4. **DocRef 精确引用**：实现文档级别的精确定位和引用
